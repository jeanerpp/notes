...
*** fetch remote pr
    ...
    rebase to PR merge: git fetch origin +refs/pull/1/merge;git switch merge_1;git rebase FETCH_HEAD -X ours
...
*** script update
POST code_hotspot/_update_by_query
{
  "script": {
    "source": """
      def name=ctx._source.filename;
      if(name!=null){
      def dirs=name.splitOnToken('/');
      def rvs=dirs[-1];
      for(int i=dirs.length-2;i>=0;i--){
          rvs=rvs+'\\'+dirs[i];
      }
      ctx._source.filename_rvs=rvs;
      }
    """,
    "lang": "painless"
  },
...
(1 row)
** enc/dec file
$ openssl enc -aes-256-cbc -in info.txt -out info -k "abc"
$ openssl enc -d -aes-256-cbc -in info -out info.txt -k "abc"
